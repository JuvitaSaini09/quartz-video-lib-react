  ...
  return (
    <>
      ...
      <section className="main-page history-page">
        <Sidebar />
        <div className="col2">
          {historyVideoState[0] !== undefined && <button className="clearHistoryBtn" onClick={()=>clearHistoryHandler()}>CLEAR ALL HISTORY</button>}
          {/* <---------History Videos-------------> */}
          <div className="videosListing mt-3">
            {historyVideoState === undefined || historyVideoState[0] === undefined ? <EmptyPage text="Empty History" /> :<>{historyVideoState.map((item) => {
              return (
                <div className="video-card">
                  ...
                  <span onClick={()=>deleteVideoFromHistoryHandler(item)} className="historyDustbin"><i className="fas fa-trash-alt fa-2x"></i></span>
